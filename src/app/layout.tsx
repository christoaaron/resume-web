...
import { getProfile, getFeaturedInsights, getFeaturedCaseStudies } from "@/lib/data";
...
export async function generateMetadata(): Promise<Metadata> {
  ...
  const baseUrl = new URL(process.env.NEXT_PUBLIC_BASE_URL || "https://christopheraaron.vercel.app");
  ...
  return {
    ...
    description: profile.bio || "Portfolio of Christopher Aaron, a dedicated Tourism, Marketing, and Business Administration graduate. Skilled in digital marketing, project management, and customer relations.",
    keywords: [
      // Personal Brand
      profile.name, `${profile.name} CV`, `${profile.name} resume`, `Tourism graduate ${profile.name}`,
      `Marketing graduate ${profile.name}`, `Business administration graduate ${profile.name}`, `Fresh graduate CV ${profile.name}`,
      `Professional portfolio ${profile.name}`, `Curriculum Vitae ${profile.name}`, `Online resume ${profile.name}`,
      // Tourism & Hospitality
    ...
    openGraph: {
      ...
      description: profile.bio || "Explore the professional portfolio of Christopher Aaron, featuring skills in tourism management, digital marketing, and business administration.",
      siteName: `${profile.name} Portfolio`,
    },
    twitter: {
      card: "summary_large_image",
      title: `${profile.name} | Portfolio`,
      description: profile.headline || "Tourism, Marketing & Business Administration Graduate.",
    },
    verification: {
